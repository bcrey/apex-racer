...
export default function App() {
    ...
    const loop = (time: number) => {
      ...
      if (Math.abs(lateralSpeed) > 3 && isOnTrack) {
        skidMarks.current.push({
          x: c.x + rightX * -10 - forwardX * 15,
          y: c.y + rightY * -10 - forwardY * 15,
          life: 1.0
        });
        skidMarks.current.push({
          x: c.x + rightX * 10 - forwardX * 15,
      ...
      // Update UI
      setSpeedKmh(Math.abs(Math.round(speed * 5)));
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-green-900 font-sans">
      ...
      <div className="absolute bottom-8 right-8 bg-black/60 text-white p-6 rounded-3xl backdrop-blur-md border border-white/10 shadow-xl flex flex-col items-end pointer-events-none">
